...
SELECT :'migration_mode' = 'apply' AS migration_apply,
       ...
       (
         ...
         OR (:'migration_dataset' = 'referents' AND :'migration_repeq' = '0' AND :'migration_referents' = '1')
       ) AS migration_selection_valid
\gset
...
\if :migration_selection_valid
\else
  \echo 'invalid migration dataset'
  \quit 2
\endif
...
\if :migration_repeq
  ...
  BEGIN
    ...
    IF invalid_count > 0 THEN
      RAISE EXCEPTION 'target representation schema misses % required columns', invalid_count;
    END IF;
    ...
    IF (SELECT representations FROM migration_expected_counts) <>
       (SELECT count(*) FROM migration_representation_raw) THEN
      RAISE EXCEPTION 'representation row count does not match the snapshot manifest';
    END IF;
...
\if :migration_referents
  ...
  BEGIN
    ...
    WHERE actual.column_name IS NULL;
    IF invalid_count > 0 THEN
      RAISE EXCEPTION 'target referent schema misses % required columns', invalid_count;
    END IF;
    ...
    IF (SELECT referents FROM migration_expected_counts) <>
       (SELECT count(*) FROM migration_referent_raw) THEN
      RAISE EXCEPTION 'referent row count does not match the snapshot manifest';
    END IF;
...
\if :migration_repeq
  ...
  CREATE TEMP TABLE migration_representation_report ON COMMIT DROP AS
  SELECT
...
\if :migration_referents
  ...
  CREATE TEMP TABLE migration_referent_report ON COMMIT DROP AS
  SELECT
...
UNION ALL SELECT 'dataset=' || :'migration_dataset';

\if :migration_repeq
  SELECT 'representations.read=' || representations_read FROM migration_representation_report
