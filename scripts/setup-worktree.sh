...
# Per-worktree secrets. Generated, never reused across worktrees, and only
# ever meaningful against the local docker stack. `openssl` is in every dev
# image we target; fall back to /dev/urandom rather than emitting a constant.
gen_secret() {
...
AUTH_SECRET_VALUE=$(gen_secret)
GATEWAY_SHARED_SECRET_VALUE=$(gen_secret)

if [ -z "$AUTH_SECRET_VALUE" ] || [ -z "$GATEWAY_SHARED_SECRET_VALUE" ]; then
  echo "[setup-worktree] Could not generate local secrets (no openssl, no /dev/urandom)." >&2
  exit 1
fi
...
# without a repo-level .env. Values match the docker-compose stack above
# (minio credentials, bucket created by the minio-init service).
AUTH_SECRET=$AUTH_SECRET_VALUE
NEXTAUTH_URL=http://localhost:$APP_DEV_PORT/api/auth
EGAPRO_GATEWAY_SHARED_SECRET=$GATEWAY_SHARED_SECRET_VALUE
S3_REGION=us-east-1
...
# External APIs are not reachable from a worktree and are not needed to
# browse the app: the SIREN lookup degrades to "Entreprise <siren>" and the
# sanction check is mocked. Point them at the local server so the URL
# validation in src/env.js passes without pretending a real endpoint exists.
EGAPRO_WEEZ_API_URL=http://localhost:$APP_DEV_PORT/unavailable/weez
EGAPRO_SUIT_API_URL=http://localhost:$APP_DEV_PORT/unavailable/suit
EGAPRO_MOCK_SUIT_SANCTION=true

# Dev-only sign-in (src/server/auth/config.ts). ProConnect registers a single
# callback URL on :3000 and needs sandbox credentials, so it cannot serve a
# worktree on a shifted port — without this, every authenticated screen is
# unreachable locally and the browser validators degrade to static checks.
