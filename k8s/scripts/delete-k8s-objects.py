...
def get_active_k8s_tags():
  ...
  for active_k8s_tag in raw_k8s_tag_list.decode('utf-8').replace('<no value>','').split('egapro-'):
    active_k8s_tag_list.append(active_k8s_tag)
  return active_k8s_tag_list

def delete_k8s_object(label):
  k8s_object_list = ["service", "ingress", "configmap", "deployments", "pod", "job"]
  for k8s_object in k8s_object_list:
    command_to_delete_k8s_object = ('kubectl delete '+ k8s_object +' --selector branch=egapro-'+label)
...
if __name__ == '__main__':
  for k8s_tag_to_delete in get_k8s_tag_to_delete(get_active_k8s_tags(), get_active_branches()):
    ...
    print('k8s objects with label branch=egapro-'+k8s_tag_to_delete+' have been deleted')
