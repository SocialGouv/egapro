...
|---|---|---|
| `block-bad-patterns.sh` | **Avant** chaque edit de fichier | Bloque 6 patterns interdits : `biome-ignore`, `@ts-ignore`, `style={}`, `<svg>` inline, `process.env` (→ `~/env.js`), `../../` (→ `~/`), `@media` en SCSS (→ mixins DSFR). L'edit est rejete, Claude doit trouver une autre approche. |
