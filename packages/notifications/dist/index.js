...
function getNotificationsDatabaseUrl() {
    ...
    const { NOTIFICATIONS_POSTGRES_USER, NOTIFICATIONS_POSTGRES_PASSWORD, NOTIFICATIONS_POSTGRES_HOST, NOTIFICATIONS_POSTGRES_PORT, NOTIFICATIONS_POSTGRES_DB, NOTIFICATIONS_POSTGRES_SSLMODE, } = process.env;
    ...
    throw new Error("NOTIFICATIONS_DATABASE_URL or NOTIFICATIONS_POSTGRES_HOST+NOTIFICATIONS_POSTGRES_DB must be set");
}
function getMainDatabaseUrl() {
    if (process.env.DATABASE_URL)
        return process.env.DATABASE_URL;
    const { POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_HOST, POSTGRES_PORT, POSTGRES_DB, POSTGRES_SSLMODE, } = process.env;
...
export function makeJobHandler(deps) {
    ...
    return async (job) => {
        ...
        try {
            ...
            if (!mailEnabled || !transporter) {
                console.log(`[notifications] MAIL_ENABLED=false — would send ${type} to ${recipientEmail}`);
            }
            else {
...
async function main() {
    ...
    console.log(`[notifications] pg-boss started (queue=${QUEUE_NAME}, mailEnabled=${mailEnabled})`);
    ...
    await boss.work(QUEUE_NAME, { batchSize: 1, includeMetadata: true }, async (jobs) => {
        const job = jobs?.[0];
        if (!job)
            return;
        try {
            ...
            console.error(`[notifications] job ${job.id} failed (attempt ${(job.retryCount ?? 0) + 1}): ${reason}`);
            throw error;
        }
    });
