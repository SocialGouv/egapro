...
export function wrapEmail({ title, intro, body }: WrapEmailInput): string {
...
export function infoList(rows: InfoRow[]): string {
	const cells = rows
		.map(
			({ label, value }) => `
<tr>
...
export function ctaButton({ label, href }: CtaInput): string {
