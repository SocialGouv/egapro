-- Migration: initial schema + full text config
...
PRIMARY KEY (siren, year));
ALTER TABLE IF EXISTS search DROP CONSTRAINT IF EXISTS declaration_exists;
ALTER TABLE IF EXISTS search ADD CONSTRAINT declaration_exists FOREIGN KEY (siren,year) REFERENCES declaration(siren,year) ON DELETE CASCADE ON UPDATE CASCADE;
...
PRIMARY KEY (siren, year));
ALTER TABLE IF EXISTS search_representation_equilibree DROP CONSTRAINT IF EXISTS representation_equilibree_exists;
ALTER TABLE IF EXISTS search_representation_equilibree ADD CONSTRAINT representation_equilibree_exists FOREIGN KEY (siren,year) REFERENCES representation_equilibree(siren,year) ON DELETE CASCADE ON UPDATE CASCADE;
