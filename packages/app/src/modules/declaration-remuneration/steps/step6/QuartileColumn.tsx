...
export function QuartileColumn({ title, quartiles }: Props) {
	return (
		<div className={stepStyles.column}>
			...
			<p className="fr-text--xs fr-mb-0">Pourcentage de femmes</p>
				...
				{quartiles.map((q) => {
					...
					return (
						<div className={stepStyles.flex1} key={`f-${q.label}`}>
							<p className="fr-text--xs fr-mb-0">{q.label}</p>
							<strong>{pct} %</strong>
						</div>
					);
				})}
			...
			<p className="fr-text--xs fr-mb-0 fr-mt-1w">Pourcentage d&apos;hommes</p>
			<div className={stepStyles.subSection}>
				{quartiles.map((q) => {
					const total = q.womenCount + q.menCount || 1;
