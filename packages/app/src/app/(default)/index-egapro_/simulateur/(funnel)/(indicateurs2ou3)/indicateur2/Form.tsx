...
import { Indicateur2Note } from "../Indicateur2Note";

type Indic2FormType = z.infer<typeof createSteps.indicateur2>;
type Indic2FormTypeWhenCalculable = Extract<Indic2FormType, { calculable: true }>;
...
export const Indic2Form = () => {
  ...
  const [lastPourcentages, setLastPourcentages] = useState<Indic2FormTypeWhenCalculable["pourcentages"]>();

  const methods = useForm<Indic2FormType>({
    mode: "onChange",
  ...
  const onSubmit = async (indicateur2: Indic2FormType) => {
    saveFunnel({ indicateur2 });
    router.push(simulateurPath(indicateur2Nav.next()));
  };
      ...
      <form noValidate onSubmit={handleSubmit(onSubmit)}>
        ...
        <ClientAnimate>
          {computableCheck && (
            <>
              <p>
                Le pourcentage de femmes et d’hommes ayant été augmentés durant la période de référence, doit être
                renseigné par CSP.
              </p>
              <AlternativeTable
                header={[
                ...
                body={categories.map<AlternativeTableProps.BodyContent>(category => ({
                  ...
                  ...(() => {
                    ...
                    const categoryError = (errors as FieldErrors<Indic2FormTypeWhenCalculable>).pourcentages?.[
                      category
                    ];
                    return {
                      cols: [
              ...
              <Box mb="4w">
                <Indicateur2Note computer={computer} resultIndicateurUn={resultIndicateurUn} />
              </Box>
            </>
          )}
        </ClientAnimate>
