...
const mocks = vi.hoisted(() => ({
	searchPublicRepresentations: vi.fn(),
...
const BASE_URL = "http://localhost/api/public/representations";
...
beforeEach(() => {
	vi.clearAllMocks();
	mocks.searchPublicRepresentations.mockResolvedValue(EMPTY_RESULT);
});

describe("GET /api/public/representations", () => {
	it("returns the search result as JSON with CORS and cache headers", async () => {
		mocks.searchPublicRepresentations.mockResolvedValue({
			data: [{ siren: "123456789", year: 2026 }],
			count: 1,
		});
	...
	it("parses and coerces every supported query parameter", async () => {
		...
		await GET(
			request(
				"?q=acme&region=11&departement=75&naf=62.01Z&year=2026&limit=25&offset=50",
			),
		);
	...
	it("returns 400 with issue details for an out-of-range limit", async () => {
		...
		expect(mocks.searchPublicRepresentations).not.toHaveBeenCalled();
	});
	...
	it("returns 500 when the service throws", async () => {
		mocks.searchPublicRepresentations.mockRejectedValue(new Error("db down"));
	...
	it("writes a success audit entry with the raw query params as metadata", async () => {
		...
		expect(mocks.logAction).toHaveBeenCalledWith(
			expect.objectContaining({
				action: "public_representations.search",
				status: "success",
				metadata: {
					q: "acme",
					region: ["11"],
...
describe("OPTIONS /api/public/representations", () => {
