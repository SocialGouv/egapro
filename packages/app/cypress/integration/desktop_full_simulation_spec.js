describe("The Home Page on desktop", function() {
  ...
  it("opens a pop-in to enter an email", function() {
    cy.get("button")
      .contains("continuer sans donner de mail")
      .click()
      .should("be.hidden");
  });
  ...
  it("displays a button that leads to the next step", function() {
    ...
    const nextStepUrl = `${currentUrl}/informations`;
    cy.get("a")
      .contains("suivant")
      .click();
  ...
  it("displays the 'informations' form", function() {
    ...
    cy.get("span")
      ...
      .click();
    cy.get("select[name=anneeDeclaration]").select("2020");
    cy.get("input[name=debutPeriodeReference]").type("26/11/2019");
  ...
  it("displays the 'recapitulatif' page", function() {
    cy.get("span")
      .contains("votre résultat total est")
      .next()
      .contains("100/100");
