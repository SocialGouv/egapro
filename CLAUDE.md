...
| `architect-rework` | Invoqué quand `e2e-dev` détecte une régression bloquante. Toujours Opus. Diagnostique la régression (diff intégré + commentaire `e2e-dev:`) et soit crée un/plusieurs **tickets Task de fix** (sous-issues To Do, sur le modèle de l'`architect` epic-enrich) que l'orchestrateur reprocesse, soit — sur **doute fonctionnel** — pose la question à l'utilisateur et **escalade** (`dispatch=escalate`). N'écrit ni code ni test. |
...
| `run_architect_rework.sh` | Invoque l'agent `architect-rework` (Opus) sur `epic/<N>` (main worktree, pas de stack) quand la gate E2E est rouge. Crée des tickets Task de fix (To Do, reprocessés par le loop) ou escalade sur doute fonctionnel (`dispatch=escalate` → exit 2 du loop). Exit 0 = tickets créés, 2 = needs_user (escaladé), 3 = rate_limited, 1 = échec. Budget Opus par défaut $10 (`EPIC_LOOP_BUDGET_REWORK`). |
