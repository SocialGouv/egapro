...
| `company` | `routers/company.ts` | `get`, `list`, `getWithDeclarations`, `updateHasCse` |
...
**SUIT — export déclarations, objet `Parcours` (v3.0.0)** : `GET /api/v1/export/declarations` (module `~/modules/export`, route `src/app/api/v1/export/declarations/route.ts`, fonction `assembleDeclaration()` dans `fetchDeclarations.ts`) regroupe, depuis la version OpenAPI `3.0.0` du contrat (`modules/export/openapi.ts`), les données déduites du parcours de la déclaration (année, effectif, tranche d'effectif, régime d'obligations, statut FSM, flags d'obligation figés, version du ruleset) sous un objet unique `Parcours` — rupture de compatibilité, URL inchangée, aucun doublon déprécié laissé à la racine. `Parcours.Prochaines_etapes_possibles` liste les transitions FSM offertes depuis `Parcours.Statut`, calculées à l'export par `listNextTransitions()` (nouveau module `src/server/rules/nextSteps.ts`) : un évaluateur **tri-valué** (`true` / `false` / `unknown`) qui élague récursivement les gardes composites (`all`/`any`/`not`) déjà décidées et ne remonte que le fait résiduel qui départagerait une garde encore `unknown`. `~/modules/export/shared/nextStepsPayload.ts` traduit chaque transition candidate en payload français (`Identifiant_transition`, `Action`, `Etat_cible`, `Libelle`, `Condition?`). Détail exhaustif du cycle de vie (8 états, table des transitions, sémantique des champs) : [`docs/SUIT-API.md`](SUIT-API.md#interprétation-des-champs-et-cycle-de-vie).
...
| EGAPRO → SUIT (CSE, GIP-MDS) | mTLS, certificat client `.p12` | `EGAPRO_SUIT_CLIENT_CERT_P12_BASE64`, `EGAPRO_SUIT_CLIENT_CERT_PASSWORD` |

Deux appels sortants sont concernés, tous deux servis par le même hôte SUIT :

| Appel | Chemin | Point de sortie | État côté SUIT |
|---|---|---|---|
| Récupération du fichier GIP-MDS | `/suit/api/externe/egapro/gipmds/latest` | `gipMds.ts` → `suitAwareFetch()` | ouvert |
| Existence d'un CSE | `/suit/api/externe/portail/CSE/{siren}` | `suit.ts` → `suitFetch()` | pas encore développé (404) |

Le second part de `src/server/services/suit.ts` via `suitFetch()`, qui construit le chemin sous `EGAPRO_SUIT_API_URL`. Le premier cible une URL **configurable** (`EGAPRO_GIP_MDS_API_URL`), qui pointe sur un mock interne hors production : il passe donc par `suitAwareFetch()`, qui n'attache le certificat que si l'URL est sur la même origine que `EGAPRO_SUIT_API_URL`. Les deux fonctions vivent dans `src/server/services/suitClient.ts`.

L'endpoint CSE n'étant pas encore développé côté SUIT, `fetchCseBySiren` reçoit un 404 et renvoie `null` — le champ `hasCse` reste simplement non renseigné. Le statut HTTP est journalisé (`[suit] CSE lookup: HTTP 404`) précisément pour le distinguer d'un échec de certificat, qui lui ne produit aucune réponse et remonte en exception. Le certificat est transporté **encodé en base64 dans une variable d'environnement** (aucun volume à monter), décodé une seule fois, et transformé en dispatcher undici mémoïsé. Ce dispatcher n'est **jamais exporté** : le lier à l'URL construite depuis `EGAPRO_SUIT_API_URL` est ce qui garantit que le certificat client ne peut être présenté qu'à SUIT, et à aucun autre hôte. Les appels posent aussi `redirect: "error"` — une redirection suivie réutilise le même dispatcher et présenterait donc le certificat à l'hôte cible ; l'API JSON de SUIT ne redirige pas, refuser une 3xx transforme une fuite silencieuse en échec journalisé. L'option `dispatcher` traverse le `fetch` patché de Next.js et n'entre pas dans la clé du cache de données : la revalidation de 24 h des deux appels est préservée.
...
| RGAA / a11y | ultra11y (Action) | `.github/workflows/a11y.yaml` · `pnpm --filter app test:a11y` | Gate statique **bloquante** (`fail-on: blocking`) sur PR ; balayage page par page au cron hebdo, non bloquant |
...
| `a11y.yaml` | PR · cron lundi 04:00 UTC · manuel | 3 jobs ultra11y : `a11y-gate` (audit RGAA statique, **bloquante**, commentaire de PR sticky), `a11y-pages` (balayage Playwright + critères de rendu + adjudication, cron/manuel seulement, non bloquant), `a11y-bundle` (fusion en un artefact `ultra11y-rgaa`) |
