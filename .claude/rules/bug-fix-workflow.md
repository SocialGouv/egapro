...
Il n'y a pas de test unitaire du pixel-perfect, mais il y a mieux qu'une revue à l'œil. Corriger en suivant `rules/figma-workflow.md` (mapping token → DSFR), puis **re-mesurer le rendu** contre le node — c'est la vérification one-shot, et elle est due comme les autres. Si l'écran est couvert par un **contrat de fidélité** (`src/e2e/fixtures/figma/<écran>.ts`), c'est lui qui verrouille : soit le code s'y conforme, soit le contrat est mis à jour avec le nouveau node en référence. Voir `rules/visual-quality-validation.md`.
