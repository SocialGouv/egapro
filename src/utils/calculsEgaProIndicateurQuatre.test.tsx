...
describe("calculIndicateurEcartNombreSalarieesAugmentees", () => {
  test("indicateurCalculable must be true", () => {
    ...
    expect(
      calculIndicateurEcartNombreSalarieesAugmentees(false, 12, 6)
    ).toEqual(undefined);
  });

  test("nombreSalarieesPeriodeAugmentation and nombreSalarieesAugmentees cant be undefined", () => {
    expect(
      calculIndicateurEcartNombreSalarieesAugmentees(true, undefined, 6)
    ).toEqual(undefined);
    expect(
      calculIndicateurEcartNombreSalarieesAugmentees(true, 6, undefined)
    ).toEqual(undefined);
    expect(
      calculIndicateurEcartNombreSalarieesAugmentees(true, undefined, undefined)
    ).toEqual(undefined);
  });

  test("nombreSalarieesPeriodeAugmentationcant be under nombreSalarieesAugmentees", () => {
    expect(
      calculIndicateurEcartNombreSalarieesAugmentees(true, 10, 11)
    ).toEqual(undefined);
    expect(calculIndicateurEcartNombreSalarieesAugmentees(true, 2, 4)).toEqual(
      undefined
    );
  });
  ...
  test("test some valid data", () => {
    ...
    expect(
      calculIndicateurEcartNombreSalarieesAugmentees(true, 20, 15)
    ).toEqual(75);
    expect(calculIndicateurEcartNombreSalarieesAugmentees(true, 5, 5)).toEqual(
      100
    );
    expect(calculIndicateurEcartNombreSalarieesAugmentees(true, 5, 0)).toEqual(
      0
    );
  });
});
